...
const filters_post = defineEventHandler(async (event) => {
    ...
    for (let key in model) {
      ...
      if (make) {
        for (let m in model[key]) {
          if (model[key][m].name_uk !== "" && !carsDB.find((f) => f.name_uk === m) && !arr.find((f) => f.name_uk === m)) {
            arr.push({
              parent_id: make.id,
              ...model[key][m]
            });
...
export { filters_post as default };
//# sourceMappingURL=filters.post.mjs.map
