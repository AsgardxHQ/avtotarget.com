    ...
    <div class="max-w-3xl mx-auto">
      ...
      <div class="w-full px-2 text-center mb-4">
        ...
        <FileUpload 
          ...
          :auto="true"
          :customUpload="true"
          @uploader="uploadImg($event, slotProps.data, '/api/upload?itemType=cars&fileType=images', 'image')"
      ...
      <div class="flex">
        <div class="md:w-1/2 sm:w-full px-2">
          <div class="text-sm">
            Название (укр):
            <input class="w-full mt-1 px-3 py-2 bg-slate-50 rounded border border-slate-400" type="text" name="name_uk" v-model="slotProps.data.name_uk">
          </div>
        </div>
        <div class="md:w-1/2 sm:w-full px-2">
          <div class="text-sm">
            Название (рус):
            <input class="w-full mt-1 px-3 py-2 bg-slate-50 rounded border border-slate-400" type="text" name="name_ru" v-model="slotProps.data.name_ru">
          </div>
        </div>
      </div>
...
const getData = async () => {
  const data = await $fetch('/api/cars', {method: "GET"});
